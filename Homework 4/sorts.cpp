...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
  ...
  for(int i = 1; i < s.size(); i++)
    {
      Store* curStore = &s.at(i);
      int curIndex = i-1;
      while(curIndex >= 0 && comp(*curStore, s.at(curIndex)))
	{
	  s.at(curIndex+1) = s.at(curIndex);
	  curIndex--;
	}
      s.at(curIndex+1) = (*curStore);
    }
}
